...
class TradfriBaseClass(Entity):
    ...
    def __init__(
        ...
        """Initialize a device."""
        self._lib_api: Callable = api  # Redefinition needed to avoid problems with timeout=, that only allowed in _api
    ...
    async def _api(self, command: Command | list[Command]) -> None:
        """Call API with command(s)."""
        try:
            await self._lib_api(command, TIMEOUT_API)
